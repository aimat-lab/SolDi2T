...
grad_EYCalc = jax.grad(EYCalc_wrapper, argnums=1)
...
def optimize_from_start(tilt_start, thickness_start):
    ...
    for step in range(num_steps):
        ...
        grad_thickness = grad_EYCalc(tilt_angle, thickness_value)
        ...
        logging.info(f"[Start ({tilt_start},{thickness_start}) | Step {step}] "
                     ...
                     f"Power: {current_power:.4f}, GradThick: {grad_thickness:.4f}, "
                     f"lr_thickness: {lr_thickness:.3f}")
...
# start_points = [(0.0, 120.0), (0.0, 200.0), (10.0, 50.0)]
start_points = [(0.0, 120.0)]
